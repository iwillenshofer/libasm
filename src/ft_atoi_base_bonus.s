...
mov		dl, byte[rdi + rcx]	; as it is inside a loop, rdi and and rcx must be
xor		rdi, rdi			;  preserved by push and pop.
...
cmp		rdx, -1
je		.end
mov		rax, r8				; this section multiplies the stored number
push	rdx					;  (r9, which is zero on the first run) by the base 
push	rcx					;  and then sums to it the position of the current
mul		r9					;  number (rdx), setting the result back to r9.
pop		rcx
pop		rdx
add		rax, rdx
mov		r9, rax
inc		rcx					; at last, increments rcx and loops, moving to the
jmp		.looping			;  next number (char) in the array (rdi).
.end:
...
mov		sil, byte[rdx + r9] ;  calls _find_n_str to check if any matches
push	rcx
...
cmp		byte[rdx + rcx], 0
jne		.loop_spaces
mov		rax, 0					; if the loop is over and there was no jump to .invalid
...
push	rsi
mov		rdi, rsi
call	_check_base			; rsi and rdi are constantly pushed and popped in 
pop		rsi
...
call	_ft_get_numbers		;  to the first position of our number (without spaces)
pop		rdx					;  or signs.
pop		rdi
mul		rdx
ret
